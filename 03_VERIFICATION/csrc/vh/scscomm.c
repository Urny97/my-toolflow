...
void scsim_comm_genfunc(scsim_communication_data_t *s) {
   ...
   s->entityName        = (char *) sc_mem_malloc(25);
   strcpy(s->entityName,  "DUMMY_MHDL_CFG_AES128_TB");
   s->architectureName  = (char *) 0;
   s->designUnitType = 19;
   s->topSimFileName    = (char *) sc_mem_malloc(37);
   strcpy(s->topSimFileName, "DEFAULT.DUMMY_MHDL_CFG_AES128_TB.sim");
   s->timebase = 2;
   s->ignorelowres = 1;
  ...
  s->jitter_enabled = 0;
   ...
   strcpy(s->CreationDate, "Sun Mar 10 17:50:04 2019");
   ...
   strcpy(s->CompilerVersion, "GNUC 4.8.5 20150623 (Red Hat 4.8.5-36)");
   ...
   s->ElaborationOptions = (char *) sc_mem_malloc(179);
   strcpy(s->ElaborationOptions, "AES128_tb -full64 -timescale=1ns/1ps -debug_all -sdf max:AES128_tb/DUT:/users/students/r0622838/digital-design-flow-aes/02_SYNTHESIS/data/AES128.synthesis.sdf -negdelay +neg_tchk");
   s->VhdlOptions = (char *) sc_mem_malloc(299);
   strcpy(s->VhdlOptions, "-mxunielab -uni_make -nc -uum -vhtop WORK.dummy_mhdl_cfg_AES128_TB -elaboration_options /tmp/vcs_20190310165001_2549/elaboptfile_2549 -picarchive -debug 4 -sdf max:AES128_tb/DUT:/users/students/r0622838/digital-design-flow-aes/02_SYNTHESIS/data/AES128.synthesis.sdf -debug_acc_options all+dmptf+bc ");
   ...
   strcpy(s->platformVersion, "Linux 3.10.0-957.5.1.el7.x86_64");
   s->platform = (char *) sc_mem_malloc(8);
   strcpy(s->platform, "linux64");
   s->vcsHome = (char *) sc_mem_malloc(41);
   strcpy(s->vcsHome, "/esat/micas-data/software/vcs-mx_2017.03");
   s->m_total_processes = 0;
   s->verWork = (char *) sc_mem_malloc(12);
   strcpy(s->verWork, "simv.daidir");
   s->scsimCompileDirName  = (char *) sc_mem_malloc(65);
   strcpy(s->scsimCompileDirName, "/users/students/r0622838/digital-design-flow-aes/03_VERIFICATION");
   s->MhdlVerilogTopCount  = 0;
   s->MhdlVerilogTopNames  = (char **) 0;
   s->MhdlVhdlTopCount  = 1;
   s->MhdlVhdlTopNames = (char **) sc_mem_malloc(8*1);
   s->MhdlVhdlTopNames[0] = "WORK.dummy_mhdl_cfg_AES128_TB";
   s->unifiedUseMod = 1;
   s->svaBindEnable = 0;
   s->isSlave = 0;
   s->lic_scsi = 0;
   ...
   s->isFsdbSet = 0;
   s->isFsdbOldSet = 0;
   s->ScsWorkDir = (char *) sc_mem_malloc(65);
   strcpy(s->ScsWorkDir, "/users/students/r0622838/digital-design-flow-aes/03_VERIFICATION");
   s->sp_mem_size = 0;
   s->proc_split_num = 0;
